...
class TRM251(Owen.OwenDevice):
    ...
    def GetTemperature(self,inputNumber=1):# Возвращает температуру на входе inputNumber, нумерация входов с единицы
        ...
        try:
            ...
            test_file=open(errorFileName,'a')
            test_file.write('Error information '+time.asctime()+'\r')
            test_file.write('TRM251::Sensor is damaged!')
            test_file.close()
            raise Exception('TRM251::Sensor is damaged!')
        except:
            test_file=open(errorFileName,'a')
            test_file.write('Error information '+time.asctime()+'\r')
            traceback.print_exc(None,test_file)
            test_file.close()
            raise Exception('TRM251::Error when getting temp!')
        return temp
