...
class OwenDevice:
    ...
    def ReadFromPort(self):#чтение ответа из порта
        ...
        if self.serialPort!=None:
            #читаем побайтово
            charsFromPort = ''
            (flag,byte) = self.serialPort.read()
            if not byte == '#':
                raise OwenProtocolError('OwenDevice::first byte is not start marker (#)!')
            charsFromPort += byte
            byteCounter = 1
            while byteCounter<self.owenProtocol.waitBytes:
                (flag,byte) = self.serialPort.read()
                if not flag:
                    raise OwenProtocolError('OwenDevice::Error occured when reading byte from serial port!')                
                if byte == '':
                    raise OwenProtocolError('OwenDevice::time out when reading byte from serial port!')
                charsFromPort += byte                
                if charsFromPort[-1] == '\r':
                    break
                byteCounter += 1
            if byteCounter == self.owenProtocol.waitBytes and not charsFromPort[-1] == '\r':
                raise OwenProtocolError('OwenDevice::read all bytes but there are no end marker!')
